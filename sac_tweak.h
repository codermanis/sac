...
#define TWEAK(T, name) static int __SAC_UNIQUE_ID = __sac_tweak_init(#name, __FILE__, type_##T); T name = __sac_tweak_##T(#name, __FILE__, __SAC_UNIQUE_ID); T __SAC_UNIQUE_NAME
...
static void __sac_handler(int sig) {}
static void* __sac_update_loop(void*);

static void __sac_tweak_global_init() {
	datas = (struct __sac_global_datas*) malloc(sizeof(struct __sac_global_datas));
	datas->fd = inotify_init();
	datas->tweak_count = 0;

	struct sigaction sa;
    sa.sa_handler = __sac_handler;
    sigemptyset(&sa.sa_mask);
    sa.sa_flags = 0;
    sigaction(SIGUSR1, &sa, NULL);

	pthread_create(&datas->th, NULL, __sac_update_loop, NULL);
...
int __sac_tweak_init(const char* name, const char* file, enum __sac_tweak_type type) {
	...
	if (!datas) {
		__sac_tweak_global_init();
	}
	raise(SIGUSR1);
	int id = datas->tweak_count++;
	assert(id < __SAC_MAX_TWEAKS);
...
static void* __sac_update_loop(void*) {
    ...
    while (1) {
	    int modified = pselect(datas->fd + 1, &fds, NULL, NULL, NULL, &set);
    	pthread_mutex_lock(&__sac_fastmutex);
